...
def _parallel_fit_per_epoch(
    ...
    for batch_idx, elem in enumerate(train_loader):
        ...
        if epoch > pre_training:
            ...
            opt1.zero_grad()
            opt2.zero_grad()
            x_pred = est1(torch.cat([data[0],target],1))
            y_pred = est2(x_pred)
            loss = criterion(y_pred, target, x_pred, *data, est1.sigma, est2.sigma)
            loss.backward()
            ...
            opt1.zero_grad()
            x_p = est1(torch.cat([data[0],target],1))
            y_p = est2(x_p)
            loss = criterion(y_p, target, x_p, data[0], est1.sigma, est2.sigma)
            loss.backward()
            opt1.step()
...
class BaggingRegressor(BaseRegressor):
    ...
    def forward(self, *x):
        ...
        outputs = [estimator[0](*x) for estimator in self.estimators_]
    ...
    def set_optimizer(self, **kwargs):
        set_module.set_optimizer(**kwargs)
    ...
    def fit(
        ...
        # Internal helper function on pesudo forward
        def _forward(estimators, *x):
            outputs = [estimator[1](*x) for estimator in estimators]
            pred = op.average(outputs)
                ...
                if test_loader:
                    ...
                    with torch.no_grad():
                        ...
                        for _, elem in enumerate(test_loader):
                            ...
                            output = _forward(estimators, *data)
                            val_loss += self._criterion(output, target, *data, *data, 1., 1.)
                        val_loss /= len(test_loader)

                        
                        if val_loss < best_loss:
                            best_loss = val_loss
                            tmp = [nn.ModuleList(e) for e in estimators]
                            self.estimators_ = nn.ModuleList()
                            self.estimators_.extend(tmp)
                        
                        train_loss = 0.0
                        for _, elem in enumerate(train_loader_):
                            data, target = io.split_data_target(
                                elem, self.device
                            )
                            ...
                            for est in estimators:
                                x_pred = est[0](torch.cat([data[0],target],1))
                                y_pred = est[1](x_pred)
                                train_loss_est += self._criterion(y_pred, target, x_pred, *data, est[0].sigma, est[1].sigma)
                            

                            train_loss += train_loss_est/len(estimators) 
                        train_loss /= len(train_loader_)
                        
                        msg = (
                            "Epoch: {:03d} | Validation MSE-Loss:"
                            " {:.5f} | Historical Best: {:.5f} | Train OD-Loss: {:.5f} "
                        )
                        self.logger.info(
                            msg.format(epoch, val_loss, best_loss, train_loss)
                        )
                        if self.tb_logger:
                            self.tb_logger.add_scalar(
